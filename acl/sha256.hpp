...
namespace acl {;
    ...
    public:
        Ref<Word> sl0(const Ref<Word> &x) {
            RefArray<Word> args = { rotr(x, 2), rotr(x, 13), rotr(x, 22) };
            return args[0]->eor(args.data(), 3);
        };
        
        Ref<Word> sl1(const Ref<Word> &x) {
            RefArray<Word> args = { rotr(x, 6), rotr(x, 11), rotr(x, 25) };
            return args[0]->eor(args.data(), 3);
        };
        
        Ref<Word> ss0(const Ref<Word> &x) {
            RefArray<Word> args = { rotr(x, 7), rotr(x, 18), shr(x, 3) };
            return args[0]->eor(args.data(), 3);
        };
        
        Ref<Word> ss1(const Ref<Word> &x) {
            RefArray<Word> args = { rotr(x, 17), rotr(x, 19), shr(x, 10) };
            return args[0]->eor(args.data(), 3);
        };
    ...
    public:
        void execute(RefArray<Word> &M, RefArray<Word> &H, Tracer<BIT>& tracer,
            ...
            for (unsigned int i = SHA256_MESSAGE_BLOCK_SIZE; i < rounds; i++) {
                RefArray<Word> W_args = { ss1(W[i-2]), W[i-7], ss0(W[i-15]), W[i-16] };
                W[i] = add(W_args);
            };
            ...
            for (unsigned int i = 0; i < rounds; i++) {
                RefArray<Word> t1_args = { h, sl1(e), ch(e, f, g), K[i], W[i] };
                Ref<Word> t1 = add(t1_args);
